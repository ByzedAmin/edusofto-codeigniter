...
<section class="panel">
	...
	<?php echo form_open('home/student_attendence'); ?>
			...
			<div class="col-md-3 mb-sm">
                <div class="form-group">
                    <label class="control-label"> <?=translate('academic_year')?> <span class="required">*</span></label>
                        <?php
                        $arrayYearD = array("" => translate('select'));
                        $years_data = $this->db->get('schoolyear')->result();
                        foreach ($years_data as $yeard) {
                            $arrayYearD[$yeard->id] = $yeard->school_year;
                        }
                        echo form_dropdown("session_id", $arrayYearD, set_value('session_id', $global_config['session_id']), "class='form-control'
                        data-plugin-selectTwo data-width='100%' data-minimum-results-for-search='Infinity' ");
                        ?>
			...
			<div class="col-md-3 mb-sm">
				<div class="form-group">
					...
					<div class="input-group">
						<input type="text" class="form-control" style="height: 38px;" name="timestamp" value="<?=set_value('timestamp', date('Y-F'))?>" data-plugin-datepicker required
	...
	<footer class="panel-footer">
		<div class="row">
			<div class="col-md-offset-10 col-md-2">
				<button type="submit" name="submit" style="color: black;background-color: gainsboro;font-size: 14px;" value="search" class="btn btn btn-default btn-block"> <i class="fas fa-filter"></i> <?=translate('filter')?></button>
			</div>
		</div>
	</footer>
    <?php echo form_close();?>
