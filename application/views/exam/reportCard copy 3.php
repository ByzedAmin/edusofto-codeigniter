...
if (count($student_array)) {
	...
	<div class="mark-container">
		...
		<table class="table table-condensed table-bordered mt-lg">
			...
			?>
				<tr>
					<td valign="middle" width="35%"><?=$row['subject_name']?><?=$row['additional_subject_code']?></td>
				<?php 
				...
				?>
				<?php if($getExam['type_id'] == 1 || $getExam['type_id'] == 3) { ?>
					<?php if ($row['additional_subject_code']) {?>
					<?php 
						...
						print($ob_mark);
						?>
					<?php $ob_mark += $total_obtain_marks?>
					<?php if($total_obtain_marks==$ob_mark) {?>
					<td valign="middle" rowspan="2"><?=$ob_mark ."/" . $total_full_marks?></td>
					<?php } ?>
					<?php }else{ ?>
