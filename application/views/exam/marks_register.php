	...
	<div class="col-md-12">
		...
		<section class="panel appear-animation" data-appear-animation="<?php echo $global_config['animations'];?>" data-appear-animation-delay="100" >
			<?php echo form_open('exam/mark_save', array('class' => 'frm-submit-msg'));
				...
				$getSchool = $this->db->where(array('id' => $branch_id))->get('branch')->row_array();
				$subject = $this->db->where(array('id' => $subject_id))->get('subject')->row_array();
				$class = $this->db->where(array('id' => $class_id))->get('class')->row_array();
				$exam = $this->db->where(array('id' => $exam_id))->get('exam')->row_array();
				$section = $this->db->where(array('id' => $section_id))->get('section')->row_array();
				// print_r($section);
			?>
			<header class="panel-heading">
			...
			<div class="panel-body">
				...
				<div class="table-responsive mt-md mb-lg" id="printableArea">
					<h3 style="text-align: center;"><?=$getSchool['school_name']?></h3>
					<p style="text-align: center;">Class : <?=$class['name']?>, Section : <?=$section['name']?></p>
					<p style="text-align: center;">Subject : <?=$subject['name']?></p>
					<p style="text-align: center;">Exam : <?=$exam['name']?></p>
							...
							<tr>
								...
								<td>
									<div class="checkbox-replace"> 
										<?php if($row['get_abs'] == 'on'){ ?>
										<label class="i-checks"><input type="checkbox" id="opacity" name="mark[<?=$key?>][absent]" <?=($row['get_abs'] == 'on' ? 'checked' : ''); ?>><i></i>Abs</label>
										<?php }else{ ?>
											<label class="i-checks"><input type="checkbox" id="opacity" name="mark[<?=$key?>][absent]" <?=($row['get_abs'] == 'on' ? 'checked' : ''); ?>><i></i></label>
										<?php } ?>
									</div>
								</td>
								<?php
