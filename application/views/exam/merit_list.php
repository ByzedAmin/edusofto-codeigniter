...
<div class="row">
	<div class="col-md-12">
		<section class="panel">
			<?php echo form_open('exam/merit_list', array('class' => 'validate')); ?>
			...
			<section class="panel appear-animation" data-appear-animation="<?php echo $global_config['animations'];?>" data-appear-animation-delay="100">
				<header class="panel-heading">
					<h4 class="panel-title">
						<i class="fas fa-users"></i> <?=translate('merit_list')?>
					</h4>
				</header>
				<div class="panel-body">
					<div class="table-responsive mt-sm mb-md">
						<div id="printResult">
							...
							<table class="table table-bordered table-hover table-condensed mb-none">
								...
								<?php 
								 $enroll = $this->db->where('total_mark ', null)->where(array( 'class_id' =>set_value('class_id'),'session_id' =>set_value('session_id'),'section_id' =>set_value('section_id')))->get('enroll')->row_array();
								?>
								<?php echo form_open('exam/merit_list_save', array('class' => 'validate')); ?>
								
								<div class="col-md-offset-10 col-md-2">
									<button type="submit" name="submit" style="margin-bottom: 20px;" value="search" class="btn btn-default btn-block"><i class="fas fa-filter"></i> <?=translate('Update List')?></button>
								</div>
								...
								<tbody>
									<?php
									$count = 1;
									$enrolls = $this->db->order_by('status', 'ASC')->order_by('total_mark', 'DESC')->get_where('enroll', array(
									...
									<tr>
										...
										<!-- <?php echo form_open('exam/merit_list_save', array('class' => 'validate')); ?>
											<input type="text" value="<?php echo set_value('class_id') ?>" name="class_id">
											<input type="text" value="<?php echo $subject['subject_id'] ?>" name="student_id">
											<input type="text" value="<?php echo set_value('exam_id') ?>" name="exam_id">
											<input type="text" value="<?php echo set_value('session_id') ?>" name="session_id">
										<?php echo form_close();?> -->
										...
										<?php
										$total_array[] =  $totalMarks;
									?>
                                         <!-- <td> -->
											<input type="hidden" value="<?php echo $totalMarks ?>" name="total_mark[]">
											<input type="hidden" value="<?php echo set_value('class_id') ?>" name="class_id">
											<input type="hidden" value="<?php echo $enroll['student_id'] ?>" name="student_id[]">
											<input type="hidden" value="<?php echo set_value('exam_id') ?>" name="exam_id">
											<input type="hidden" value="<?php echo set_value('session_id') ?>" name="session_id">
											<input type="hidden" value="<?php echo set_value('section_id') ?>" name="section_id">
											<?php if ($unset_subject == 0) { ?>
												<?php if($result_status) { ?>
												<input type="hidden" value="<?php echo $unset_subject ?>" name="status[]">
											<?php }else{ ?>
												<input type="hidden" value="1" name="status[]">
											<?php } ?>
											<?php } ?>
										<!-- </td> -->
										<td><?php echo ($totalMarks . '/' . $totalFullmarks); ?>
