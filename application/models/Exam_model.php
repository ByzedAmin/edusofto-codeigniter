...
class Exam_model extends CI_Model
{
    ...
    public function getSubjectList($examID, $classID, $sectionID, $sessionID)
    {
        ...
        $this->db->select('t.*,s.name as subject_name,s.cut_point,s.add_subject_code,s.sub_mark');
    ...
    public function getNSubjectList($examID, $classID, $sectionID, $sessionID)
    {
        $branchID = $this->application_model->get_branch_id();
        $this->db->select('t.*,s.name as subject_name,s.sub_mark');
        $this->db->from('timetable_exam as t');
        $this->db->join('subject as s', 's.id = t.subject_id', 'inner');
        $this->db->where('t.exam_id', $examID);
        $this->db->where('t.class_id', $classID);
        ...
        $this->db->where('t.session_id', $sessionID);
        $this->db->where('t.branch_id', $branchID);
        $this->db->where('s.sub_mark!=', null);
        $this->db->group_by('t.subject_id');
        return $this->db->get()->result_array();
    }
    public function getOpSubjectList($examID, $classID, $sectionID, $sessionID)
    {
        $branchID = $this->application_model->get_branch_id();
        $this->db->select('t.*,s.name as subject_name,s.subject_type,s.cut_point');
        $this->db->from('timetable_exam as t');
        $this->db->join('subject as s', 's.id = t.subject_id', 'inner');
        $this->db->where('t.exam_id', $examID);
        $this->db->where('t.class_id', $classID);
        ...
        $this->db->where('t.session_id', $sessionID);
        $this->db->where('t.branch_id', $branchID);
        $this->db->where('s.subject_type', 'Optional');
        $this->db->group_by('t.subject_id');
        return $this->db->get()->result_array();
    }
    ...
    public function getStudentReportCard($studentID, $examID, $sessionID)
    {
        ...
        $this->db->select('m.mark as get_mark,IFNULL(m.absent, 0) as get_abs,subject.name as subject_name,subject.add_subject_code as additional_subject_code,subject.sub_mark, te.mark_distribution');
        ...
        $this->db->select('m.mark as get_mark,IFNULL(m.absent, 0) as get_abs,subject.name as subject_name,subject.add_subject_code as additional_subject_code,subject.sub_mark,subject.cut_point, te.mark_distribution');
        $this->db->from('mark as m');
        $this->db->join('subject', 'subject.id = m.subject_id', 'left');
        $this->db->join('timetable_exam as te', 'te.exam_id = m.exam_id and te.class_id = m.class_id and te.section_id = m.section_id and te.subject_id = m.subject_id', 'left');
        $this->db->where('m.exam_id', $examID);
        ...
        $this->db->where('m.session_id', $sessionID);
        $this->db->where('subject.subject_type', 'Optional');
        // $this->db->where('m.mark !=', null);
        $result['op_exam'] = $this->db->get()->result_array();
        return $result;
    }
