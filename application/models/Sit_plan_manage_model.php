...
class Sit_plan_manage_model extends MY_Model
{
    ...
    {
        $this->db->select('sit_plan_templete.*,branch.name as branchname');
        $this->db->from('sit_plan_templete');
        $this->db->join('branch', 'branch.id = sit_plan_templete.branch_id', 'left');
        if (!is_superadmin_loggedin()) {
            $this->db->where('sit_plan_templete.branch_id', get_loggedin_branch_id());
        }
        $this->db->where('sit_plan_templete.card_type', $type);
        $this->db->order_by('sit_plan_templete.id', 'ASC');
    ...
    public function save($data)
    {
        ...
        if (!isset($data['templete_id'])) {
            $this->db->insert('sit_plan_templete', $arrayLive);
        } else {
            $this->db->where('id', $data['templete_id']);
            $this->db->update('sit_plan_templete', $arrayLive);
        }
    }
