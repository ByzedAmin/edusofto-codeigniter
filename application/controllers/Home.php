...
class Home extends Frontend_Controller
{
    ...
    public function notice()
    {
        ...
        $this->data['main_contents'] = $this->load->view('home/notice', $this->data, true);
        $this->load->view('home/layout/index', $this->data);
    }
    ...
    public function admitCardprintFn()
    {
        if ($_POST) {
            ...
            if ($this->form_validation->run() == true) {
                ...
                $userID = $this->db->select('id')->where('roll', $registerNo)->where('branch_id', $branchID)->where('session_id', $sessionID)->where('section_id', $sectionId)->where('class_id', $classID)->get('enroll')->row_array();
                if (empty($userID)) {
    ...
    public function examResultsPrintFn()
    {
        ...
        if ($_POST) {
            ...
            if ($this->form_validation->run() == true) {
                ...
                $classID = $this->input->post('class_id');
                $sectionId = $this->input->post('section');
                $userID = $this->db->select('student_id')->where('roll', $registerNo)->where('branch_id', $branchID)->where('session_id', $sessionID)->where('section_id', $sectionId)->where('class_id', $classID)->get('enroll')->row_array();
                if (empty($userID)) {
                    $array = array('status' => '0', 'error' => "Register No Not Found.");
                    echo json_encode($array);
                    exit();
                }
                $result = $this->exam_model->getStudentReportCard($userID['student_id'], $examID, $sessionID);
