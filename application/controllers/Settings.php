...
class Settings extends Admin_Controller
{
    ...
    public function universal()
    {
        ...
        if ($this->input->post('submit') == 'logo') {
            if (isset($_FILES["logo_file"]) && !empty($_FILES["logo_file"]['name'])) {
                $imageNmae = $_FILES['logo_file']['name'];
                $extension = pathinfo($imageNmae, PATHINFO_EXTENSION);
                $newLogoName = "fav_icon".time().date('md')."." . $extension;
                $image_path = './uploads/frontend/images/' . $newLogoName;
                if (move_uploaded_file($_FILES['logo_file']['tmp_name'], $image_path)) {
                    $cms_setting['global_logo'] = $newLogoName;
                }
            }
            if (isset($_FILES["text_logo"]) && !empty($_FILES["text_logo"]['name'])) {
                $imageNmae = $_FILES['text_logo']['name'];
                $extension = pathinfo($imageNmae, PATHINFO_EXTENSION);
                $newLogoName = "text_logo".time().date('md')."." . $extension;
                $image_path = './uploads/frontend/images/' . $newLogoName;
                if (move_uploaded_file($_FILES['text_logo']['tmp_name'], $image_path)) {
                    $cms_setting['text_logo'] = $newLogoName;
                }
            }
            if (isset($_FILES["print_file"]) && !empty($_FILES["print_file"]['name'])) {
                $imageNmae = $_FILES['print_file']['name'];
                $extension = pathinfo($imageNmae, PATHINFO_EXTENSION);
                $newLogoName = "print_file".time().date('md')."." . $extension;
                $image_path = './uploads/frontend/images/' . $newLogoName;
                if (move_uploaded_file($_FILES['print_file']['tmp_name'], $image_path)) {
                    $cms_setting['print_file'] = $newLogoName;
                }
            }
            if (isset($_FILES["report_card"]) && !empty($_FILES["report_card"]['name'])) {
                $imageNmae = $_FILES['report_card']['name'];
                $extension = pathinfo($imageNmae, PATHINFO_EXTENSION);
                $newLogoName = "report_card".time().date('md')."." . $extension;
                $image_path = './uploads/frontend/images/' . $newLogoName;
                if (move_uploaded_file($_FILES['report_card']['tmp_name'], $image_path)) {
                    $cms_setting['report_card'] = $newLogoName;
                }
            }
            if (isset($_FILES["slider_1"]) && !empty($_FILES["slider_1"]['name'])) {
                $imageNmae = $_FILES['slider_1']['name'];
                $extension = pathinfo($imageNmae, PATHINFO_EXTENSION);
                $newLogoName = "slider_1".time().date('md')."." . $extension;
                $image_path = './uploads/frontend/images/' . $newLogoName;
                if (move_uploaded_file($_FILES['slider_1']['tmp_name'], $image_path)) {
                    $cms_setting['slider_1'] = $newLogoName;
                }
            }
            if (isset($_FILES["slider_2"]) && !empty($_FILES["slider_2"]['name'])) {
                $imageNmae = $_FILES['slider_2']['name'];
                $extension = pathinfo($imageNmae, PATHINFO_EXTENSION);
                $newLogoName = "slider_2".time().date('md')."." . $extension;
                $image_path = './uploads/frontend/images/' . $newLogoName;
                if (move_uploaded_file($_FILES['slider_2']['tmp_name'], $image_path)) {
                    $cms_setting['slider_2'] = $newLogoName;
                }
            }
            if (isset($_FILES["slider_3"]) && !empty($_FILES["slider_3"]['name'])) {
                $imageNmae = $_FILES['slider_3']['name'];
                $extension = pathinfo($imageNmae, PATHINFO_EXTENSION);
                $newLogoName = "slider_3".time().date('md')."." . $extension;
                $image_path = './uploads/frontend/images/' . $newLogoName;
                if (move_uploaded_file($_FILES['slider_3']['tmp_name'], $image_path)) {
                    $cms_setting['slider_3'] = $newLogoName;
                }
            }
            if (isset($_FILES["sidebox_1"]) && !empty($_FILES["sidebox_1"]['name'])) {
                $imageNmae = $_FILES['sidebox_1']['name'];
                $extension = pathinfo($imageNmae, PATHINFO_EXTENSION);
                $newLogoName = "sidebox_1".time().date('md')."." . $extension;
                $image_path = './uploads/frontend/images/' . $newLogoName;
                if (move_uploaded_file($_FILES['sidebox_1']['tmp_name'], $image_path)) {
                    $cms_setting['sidebox_1'] = $newLogoName;
                }
            }
            if (isset($_FILES["profile_bg"]) && !empty($_FILES["profile_bg"]['name'])) {
                $imageNmae = $_FILES['profile_bg']['name'];
                $extension = pathinfo($imageNmae, PATHINFO_EXTENSION);
                $newLogoName = "profile_bg".time().date('md')."." . $extension;
                $image_path = './uploads/frontend/images/' . $newLogoName;
                if (move_uploaded_file($_FILES['profile_bg']['tmp_name'], $image_path)) {
                    $cms_setting['profile_bg'] = $newLogoName;
                }
            }

            $this->db->where('id', 1);
            $this->db->update('global_settings', $cms_setting);
            // set_alert('success', translate('the_configuration_has_been_updated'));
            // move_uploaded_file($_FILES['logo_file']['tmp_name'], 'uploads/app_image/logo'.time().date('md').'.png');
            // move_uploaded_file($_FILES['text_logo']['tmp_name'], 'uploads/app_image/logo-small'.time().date('md').'.png');
            // move_uploaded_file($_FILES['print_file']['tmp_name'], 'uploads/app_image/printing-logo'.time().date('md').'.png');
            // move_uploaded_file($_FILES['report_card']['tmp_name'], 'uploads/app_image/report-card-logo'.time().date('md').'.png');

            // move_uploaded_file($_FILES['slider_1']['tmp_name'], 'uploads/login_image/slider_1'.time().date('md').'.jpg');
            // move_uploaded_file($_FILES['slider_2']['tmp_name'], 'uploads/login_image/slider_2'.time().date('md').'.jpg');
            // move_uploaded_file($_FILES['slider_3']['tmp_name'], 'uploads/login_image/slider_3'.time().date('md').'.jpg');

            // move_uploaded_file($_FILES['sidebox_1']['tmp_name'], 'assets/login_page/image/sidebox'.time().date('md').'.jpg');
            // move_uploaded_file($_FILES['profile_bg']['tmp_name'], 'assets/images/profile_bg'.time().date('md').'.jpg');
    ...
    public function bkash_save()
    {
        ...
        $this->form_validation->set_rules('bkash_secret_key', 'bKash Secret Key', 'trim|required');
        ...
        if ($this->form_validation->run() !== false) {
            $bkashSandbox = isset($_POST['bkash_sandbox']) ? 1 : 2;
            $arrayBkash = array(
                'bkash_app_key' => $this->input->post('bkash_app_key'),
                'bkash_secret_key' => $this->input->post('bkash_secret_key'),
