...
class Card_manage extends Admin_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->load->model('sit_plan_manage_model');
    ...
    public function sit_plan_templete()
    {
        if (!get_permission('id_card_templete', 'is_view')) {
            access_denied();
        }
        if ($_POST) {
            if (get_permission('id_card_templete', 'is_add')) {
                ...
                if ($this->form_validation->run() !== false) {
                    ...
                    $post = $this->input->post();
                    $post['card_type'] = 1;
                    $this->sit_plan_manage_model->save($post);
                    set_alert('success', translate('information_has_been_saved_successfully'));
                    $array = array('status' => 'success');
                } else {
                    $error = $this->form_validation->error_array();
        ...
        $this->data['branch_id'] = $this->application_model->get_branch_id();
        $this->data['certificatelist'] = $this->sit_plan_manage_model->getList();
        $this->data['title'] = translate('sit_plan') . " " . translate('templete');
        $this->data['sub_page'] = 'card_manage/sit_plan_template';
        $this->data['main_menu'] = 'card_manage';
        $this->load->view('layout/index', $this->data);
    }
    ...
    public function sit_plan_templete_edit($id = '')
    {
        if (!get_permission('id_card_templete', 'is_edit')) {
            access_denied();
        }
        if ($_POST) {
            ...
            if ($this->form_validation->run() !== false) {
                ...
                $this->sit_plan_manage_model->save($post);
                set_alert('success', translate('information_has_been_updated_successfully'));
                $url = base_url('card_manage/sit_plan_templete');
                $array = array('status' => 'success', 'url' => $url);
            } else {
                $error = $this->form_validation->error_array();
                $array = array('status' => 'fail', 'error' => $error);
        ...
        $this->data['certificate'] = $this->app_lib->getTable('sit_plan_templete', array('t.id' => $id), true);
        $this->data['title'] = translate('sit_plan') . " " . translate('templete');
        $this->data['headerelements'] = array(
            'css' => array(
                'css/certificate.css',
        ...
        $this->data['sub_page'] = 'card_manage/sit_plan_templete_edit';
        $this->data['main_menu'] = 'card_manage';
        $this->load->view('layout/index', $this->data);
    }
    ...
    public function sit_plan_delete($id = '')
    {
        if (get_permission('id_card_templete', 'is_delete')) {
            if (!is_superadmin_loggedin()) {
                $this->db->where('branch_id', get_loggedin_branch_id());
            ...
            $this->db->where('id', $id);
            $getRow = $this->db->get('sit_plan_templete')->row_array();
            if (!empty($getRow)) {
                $path = 'uploads/certificate/';
                if (file_exists($path . $getRow['background'])) {
                    unlink($path . $getRow['background']);
                ...
                $this->db->where('id', $id);
                $this->db->where('card_type', 1);
                $this->db->delete('sit_plan_templete');
            }
        }
    }
    ...
    public function getSitPlan()
    {
        if (get_permission('id_card_templete', 'is_view')) {
            $templateID = $this->input->post('id');
            $this->data['template'] = $this->sit_plan_manage_model->get('sit_plan_templete', array('id' => $templateID), true);
            $this->load->view('card_manage/viewIDCard', $this->data);
        }
    }
    ...
    public function generate_sit_plan()
    {
        if (!get_permission('generate_student_idcard', 'is_view')) {
            access_denied();
        }
        ...
        $this->data['branch_id'] = $this->application_model->get_branch_id();
        $this->data['title'] = translate('student') . " " . translate('sit_plan') . " " . translate('generate');
        $this->data['sub_page'] = 'card_manage/generate_sit_plan';
        $this->data['main_menu'] = 'card_manage';
        $this->load->view('layout/index', $this->data);
    }
    ...
    public function sitPlanprintFn($opt = '')
    {
        if ($_POST) {
            if ($opt == 1) {
                if (!get_permission('generate_student_idcard', 'is_view')) {
            ...
            $this->data['print_date'] = $this->input->post('print_date');
            $this->data['expiry_date'] = $this->input->post('expiry_date');
            echo $this->load->view('card_manage/sitPlanprintFn', $this->data, true);
        }
    }
