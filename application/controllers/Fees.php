...
class Fees extends Admin_Controller
{
    ...
    public function paymentRevert()
    {
        ...
        foreach ($ids as $key => $value) {
            ...
            // $q = $this->db->get('fee_payment_history');
            // $data = $q->result_array();

            $data = $this->db->where('id', $value)->get('fee_payment_history')->row_array();

            // $this->fees_model->payment_revert($data);

            $voucher = $this->db->where(array('name' => 'Student Fees Collection','system' => 1))->get('voucher_head')->row_array();


            $bal = $data['amount'];

            $transaction = $this->db->where('date',$data['date'])->where(array( 'voucher_head_id' =>$voucher['id']))->get('transactions')->row_array();
            $account = $this->db->where(array( 'id' =>$transaction['account_id']))->get('accounts')->row_array();
            $balance = $account['balance']-$bal;
            $arrayAccount = array(
                'balance' => $balance,
            );
            
            $this->db->where('id', $transaction['account_id']);
            $this->db->update('accounts', $arrayAccount);

            $transaction = $this->db->where('date',$data['date'])->where(array( 'voucher_head_id' =>$voucher['id']))->get('transactions')->row_array();
            $transaction2 = $this->db->where('date',$data['date'])->where(array( 'voucher_head_id' =>$voucher['id']))->get('transactions')->row_array();
            $amount = $transaction2['amount']-$bal;
            $credit = $transaction2['cr']-$bal;
            ...
            $this->db->update('transactions', $arrayAllocation);
