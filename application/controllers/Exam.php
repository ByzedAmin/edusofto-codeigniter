...
class Exam extends Admin_Controller
{
    ...
    public function tabulation_sheet()
    {
        ...
        if (!empty($this->input->post('submit'))) {
            ...
            $this->data['get_n_subjects'] = $this->exam_model->getNSubjectList($examID, $classID, $sectionID, $sessionID);
            $this->data['get_op_subjects'] = $this->exam_model->getOpSubjectList($examID, $classID, $sectionID, $sessionID);
        }
    ...
    public function merit_list()
    {
        ...
        $this->data['title'] = translate('merit_list');
        $this->data['sub_page'] = 'exam/merit_list';
        $this->data['main_menu'] = 'exam_reports';
        $this->load->view('layout/index', $this->data);
    }

    public function merit_list_save(){
        ...
        foreach($total_mark as $key=>$value){
            // print_r($value);
            $enroll = $this->db->where('student_id',$student_id[$key])->where(array( 'class_id' =>$class_id,'session_id' =>$session_id,'section_id' =>$section_id))->get('enroll')->row_array();
            
            $arrayAccount = array(
                'total_mark' => $value,
                'status' => $status[$key],
            );
            $this->db->where('id', $enroll['id']);
            $result = $this->db->update('enroll', $arrayAccount);
            // var_dump($enroll);
            
        }
        // return $result;
        redirect('exam/merit_list');
