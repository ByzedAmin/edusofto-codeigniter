...
class Admissionpayment extends Frontend_Controller
{
    ...
    public function bkash()
    {
        ...
        if (!empty($params)) {
            if ($config['bkash_username'] == "" || $config['bkash_password'] == "" || $config['bkash_app_key'] == "" || $config['bkash_secret_key'] == "" || $config['bkash_status'] == "") {
                ...
                $data = array(
                    ...
                    'token_url' => base_url('admissionpayment/bkash_token'),
                    'checkout_url' => base_url('admissionpayment/bkash_checkout'),
                    'execute_url' => base_url('admissionpayment/bkash_execute'),
                    'success_url' => base_url('admissionpayment/bkash_success'),
                );

                $this->load->view('layout/bkash', $data);
            }
        }
    }
    ...
    {
        echo $this->bkash_payment->getAccessToken();
    }

    public function bkash_checkout()
    {
        $stream_clean = $this->security->xss_clean($this->input->raw_input_stream);
        $request = json_decode($stream_clean, true);

        if ($this->input->server('REQUEST_METHOD') == 'POST' && !empty($stream_clean)) {
            $data = [
                'token' => $request['token'],
                'amount' => $request['amount'],
                'currency' => $request['currency'],
                'intent' => 'sale',
                'invoice_no' => $request['merchantInvoiceNumber'],
                'student_name' => $request['merchantAssociationInfo'],
            ];

            echo $this->bkash_payment->createCheckoutRequest($data);
        }else{
    ...
    public function bkash_execute()
    {
        ...
        if ($this->input->server('REQUEST_METHOD') == 'POST' && !empty($stream_clean)) {
            ...
            echo $this->bkash_payment->executePayment($token, $paymentId);
        }else{
            echo "Invalid Request";
        }
    }
    ...
    public function bkash_success()
    {
        ...
        if (!empty($token) && !empty($paymentId) && !empty($params)) {
            try {
               $response = $this->bkash_payment->verifyPayment($token, $paymentId);
                $response = json_decode($response, true);

                if (isset($response['transactionStatus']) && $response['transactionStatus'] == 'Completed') {
                    $amount = floatval($response['amount']);
                    $ref_id = $response['trxID'];
                    // payment info update in invoice
                    $arrayFees = array(
                        'data' => $params,
                        'amount' => $amount,
                        'remarks' => "Admission Fees deposits online via bKash Ref ID: " . $ref_id,
        ...
        if (!empty($params)) {
            if ($config['razorpay_key_id'] == "" || $config['razorpay_key_secret'] == "") {
                ...
                $arrayData = array(
                    ...
                    'image' => base_url('uploads/frontend/images/'.$global_config['text_logo']),
                    'currency' => 'INR',
